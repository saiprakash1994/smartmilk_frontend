...
function generateMatrixTable(
  ...
  const matrix = [];
  const header = [
    stepType === "FAT + CLR" ? "f/c" : "f/s",
    ...snfValues.map((s) => s.toFixed(1)),
  ];
  matrix.push(header);
  let prevFatRow = null;
...
const PriceTableGenerator = () => {
  ...
  const validateFatRulesEditing = (
    ...
    if (
      rules.length > 0 &&
      Number(Number(rules[0].from).toFixed(1)) !==
        Number(Number(start).toFixed(1))
    ) {
    ...
    for (let i = 0; i < rules.length; ++i) {
      ...
      if (i > 0) {
        ...
        if (Math.abs(from - (prevTo + 0.1)) > 0.0001) {
          return `FAT step #${i + 1} must start at ${(prevTo + 0.1).toFixed(
            1
          )} (immediately after previous rule's end).`;
        }
      }
    }
    return "";
  ...
  const validateSnfRulesEditing = (
    ...
    for (let i = 0; i < rules.length; ++i) {
      ...
      if (to > end) {
        return `${snfOrClrLabel} step #${
          i + 1
        } 'To' must not exceed ${snfOrClrLabel} End (${end}).`;
      }
      if (i > 0) {
        const prevTo = Number(Number(rules[i - 1].to).toFixed(1));
        if (from < prevTo) {
        ...
        if (Math.abs(from - (prevTo + 0.1)) > 0.0001) {
          return `${snfOrClrLabel} step #${i + 1} must start at ${(
            prevTo + 0.1
          ).toFixed(1)} (immediately after previous rule's end).`;
        }
      }
    }
    return "";
  ...
  const validateFatRules = (
    ...
    if (
      rules.length > 0 &&
      Number(Number(rules[rules.length - 1].to).toFixed(1)) !==
        Number(Number(end).toFixed(1))
    ) {
  ...
  const handleGenerate = (e) => {
    ...
    if (fatRules.length === 0 || snfRules.length === 0) {
      setError(
        "Please add at least one FAT and one " +
          snfOrClrLabel +
          " step before generating the table."
      );
      return;
    }
  ...
  const canAddFatRule =
    fatRules.length === 0 ||
    Number(Number(fatRules[fatRules.length - 1].to).toFixed(1)) <
      Number(Number(fatEnd).toFixed(1));
  const canAddSnfRule =
    snfRules.length === 0 ||
    Number(Number(snfRules[snfRules.length - 1].to).toFixed(1)) <
      Number(Number(snfEnd).toFixed(1));
          ...
          <div className="d-flex w-100 align-items-center justify-content-between position-relative">
            <div className="d-flex flex-column">
              <span className="d-flex align-items-center mb-1">
                ...
                >
                  Milk Rate Table Generator
                </span>
              </span>
              <div
                style={{ fontSize: "1rem", opacity: 0.9, textAlign: "left" }}
              >
                Generate, download and upload milk rate tables with custom steps
              </div>
            </div>
                  ...
                  <Card className=" ratetable-page config-section-card fat-config-card mb-4 w-100">
                    ...
                    <Card.Body>
                      ...
                      {fatRuleError && (
                        <Alert
                          variant="danger"
                          className="py-1 px-2 mb-2 d-flex align-items-center"
                        >
                                ...
                                <tr key={idx}>
                                  ...
                                  <td>
                                    <Button
                                      ...
                                      size="sm"
                                      onClick={() => handleRemoveFatRule(idx)}
                                      title="Remove Step"
                                    >
                                      <FaTrash />
                                    </Button>
                                ...
                                <tr key={idx}>
                                  ...
                                  <td>
                                    <Button
                                      variant="outline-danger"
                                      size="sm"
                                      onClick={() => handleRemoveSnfRule(idx)}
                                      title="Remove Step"
                                    >
                                      <FaTrash />
                                    </Button>
                  ...
                  <thead className="sticky-header">
                    <tr>
                      {matrixTable[0].map((col, idx) => (
                        ...
                        >
                          {idx === 0
                            ? `${snfOrClrLabel === "CLR" ? "f/c" : "f/s"}`
                            : idx > 0
                            ? `${
                                snfOrClrLabel === "CLR"
                                  ? parseFloat(col).toFixed(1)
                                  : col
                              }`
                            : col}
                        </th>
